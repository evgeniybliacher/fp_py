...
from fp_py.Either import Try
from assertpy import assert_that,fail



@pytest.mark.trymonad
def test_try_right():
    res = Try.unit(6).bind(lambda x: x/3).bind(lambda x: x+7)
    assert_that(res.is_right()).is_true()
    assert_that(res.unwrap()).is_equal_to(9)
    v = res.match(
        left= lambda x: f"Exception {str(x)}",
        right= lambda x: x
    )
    print(v)
...
@pytest.mark.trymonad
def test_try_left():
    res = Try.unit(0).bind(lambda x: 6/x).bind(lambda x: x+7)
    assert_that(res.is_left()).is_true()
    v = res.match(
        left= lambda x: f"Exception {str(x)}",
        right= lambda x: x
    )
